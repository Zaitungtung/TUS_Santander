...
public class MaterialSearchView extends FrameLayout implements Filter.FilterListener {
    ...
    private void initiateView() {
        ...
        mSuggestionsRecycler = (RecyclerView) mSearchLayout.findViewById(R.id.suggestion_list);
        ...
        mSuggestionsRecycler.setLayoutManager(mLayoutManager);

        mSuggestionsRecycler.setVisibility(GONE);
    ...
    private void initClickListeners() {
        ...
        mSuggestionsRecycler.setOnClickListener(mOnClickListener);
    ...
    private void setFilterIcon(Drawable drawable) {
        mFilterBtn.setImageDrawable(drawable);
    }

    private void setSuggestionIcon(Drawable drawable) {
        Drawable suggestionIcon = drawable;
    }

    private void setSuggestionBackground(Drawable background) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) {
            mSuggestionsRecycler.setBackground(background);
        } else {
            mSuggestionsRecycler.setBackgroundDrawable(background);
        }
    }

    private void setHistoryIcon(Drawable drawable) {
        Drawable historyIcon = drawable;
    }
     ...
     * #setAdapter(SearchAdapter)} before calling this.
     */
    private void showSuggestions() {
        if (mAdapter != null && mAdapter.getItemCount() > 0 && mSuggestionsRecycler.getVisibility() == GONE && isAnimationShown) {
            ViewCompat.jumpDrawablesToCurrentState(mSuggestionsRecycler);
            mSuggestionsRecycler.setVisibility(VISIBLE);
        }
    }
     ...
     */
    private void setAdapter(SearchAdapter adapter) {
        mAdapter = adapter;
        mSuggestionsRecycler.setAdapter(adapter);
    }

    /**
     * Set Adapter for suggestions list with the given suggestion array
     *
     * @param suggestionsName array of suggestions
     */
    public void setSuggestions(List<String> suggestionsName, List<String> suggestionsNumber,
                               List<String> type) {
        Type = type;
        setSuggestionsAndHistory(suggestionsName, suggestionsNumber, mHistory);
    }
     ...
     *
     * @param suggestionsName   array of suggestions
     * @param suggestionsNumber array of suggestions
     * @param history           array of history
     */
    private void setSuggestionsAndHistory(List<String> suggestionsName, List<String> suggestionsNumber,
                                          String[] history) {

        if ((suggestionsName != null && suggestionsName.size() > 0)) {
            mTintView.setVisibility(VISIBLE);
            final SearchAdapter adapter = new SearchAdapter(mContext, suggestionsName, suggestionsNumber,
                    Type, ellipsize, OnItemClickListener);
            setAdapter(adapter);
    ...
    private void dismissSuggestions() {
        if (mSuggestionsRecycler.getVisibility() == VISIBLE) {
            mSuggestionsRecycler.setVisibility(GONE);
        }
    }
    ...
    private void setAnimationDuration() {
        mAnimationDuration = AnimationUtil.ANIMATION_DURATION_MEDIUM;
    }
     ...
     */
    private void showSearch(boolean animate, boolean showSuggestionsAfterAnimation) {
        if (isSearchOpen()) {
            return;
        }
    ...
    private void setVisibleWithAnimation(final boolean showSuggestions) {
        AnimationUtil.AnimationListener animationListener = new AnimationUtil.AnimationListener() {
            ...
            @Override
            public boolean onAnimationCancel() {
                return false;
            }
        };
    ...
    private void setTintVisibility(int visibility) {
